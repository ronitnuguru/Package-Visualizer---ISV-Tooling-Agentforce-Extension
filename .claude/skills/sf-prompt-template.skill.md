...
### ⚠️ CRITICAL: Deploying New vs Existing Templates

**For NEW templates (first deployment):**
- **OMIT** `activeVersionIdentifier` completely  
- **OMIT** `versionIdentifier` completely
- Salesforce auto-generates both and activates version 1

**Common errors if you include custom identifiers:**
```
❌ The prompt template version identifier is "v1_error_debugger" invalid
❌ Select an active version of the Package_Push_Error_Debugger prompt template
```

**Correct new template structure:**
```xml
<GenAiPromptTemplate xmlns="http://soap.sforce.com/2006/04/metadata">
    <!-- NO activeVersionIdentifier field -->
    <description>Template description</description>
    <developerName>My_New_Template</developerName>
    <templateVersions>
        <content>Prompt here</content>
        <inputs>...</inputs>
        <status>Published</status>
        <!-- NO versionIdentifier field -->
    </templateVersions>
    <type>einstein_gpt__flex</type>
</GenAiPromptTemplate>
```
...
**Deployment workflow:**
1. Create template WITHOUT identifiers
2. Deploy: `sf project deploy start --source-dir force-app/main/default/genAiPromptTemplates`
3. Retrieve: `sf project retrieve start --metadata GenAiPromptTemplate:My_New_Template`
4. Commit retrieved file (now has hashes like `/x/cQ0tDr6f5AD75j+9520LBwOtiual/FJhY7zpVmUI=_1`)

**For EXISTING templates (updates):**
- Keep identifiers from retrieved metadata  
- Only modify `<content>`, `<inputs>`, or `<primaryModel>`
...
#### 7. INVALID_VERSION_IDENTIFIER (Deployment)
**Symptom**: 
```
The prompt template version identifier is "v1_error_debugger" invalid
Select an active version of the Package_Push_Error_Debugger prompt template
```
**Cause**: Using custom strings for `versionIdentifier` or `activeVersionIdentifier` instead of system-generated hashes  
**Solution**: For NEW templates, omit both fields entirely:
```xml
<!-- ✅ CORRECT -->
<GenAiPromptTemplate>
    <!-- NO activeVersionIdentifier -->
    <developerName>My_Template</developerName>
    <templateVersions>
        <status>Published</status>
...
```
Deploy, then retrieve to get system-generated hashes. See "Deploying New vs Existing Templates" section.
